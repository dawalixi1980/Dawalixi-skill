...
5. **PowerShell 5.1 (all modules):** If running in PowerShell, detect version before first API call. PS 5.1 silently converts request Body to GBK — must use UTF-8 byte array mode. See [Detailed Rules](#powershell-51-environment-detection).
  ...
  6. **API pagination (all modules):** ALL APIs returning paginated results (containing `is_end` and `next_cursor`) MUST be looped until `is_end=true`. NEVER proceed with only the first page. Collect ALL pages before making decisions or presenting to the user. This applies to: `search_knowledge_base`, `get_knowledge_list`, `search_knowledge`, `get_addable_knowledge_base_list`, `list_note`, `list_notebook`, `search_note`.
