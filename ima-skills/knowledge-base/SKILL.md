...
所有返回分页结果的 API（响应中包含 `is_end` 和 `next_cursor` 字段），**必须**循环翻页直到 `is_end=true`，拿到**全部数据**后再进行下一步操作。禁止只读取第一页就做出判断或展示给用户。

涉及分页的接口：
- `search_knowledge_base` — 搜索/列出知识库，每页上限 20 条
- `get_knowledge_list` — 获取知识库内容列表，每页上限 50 条
- `search_knowledge` — 在知识库内搜索，每页上限 20 条
- `get_addable_knowledge_base_list` — 可添加的知识库，每页上限 50 条

**翻页示例流程：**
1. 调用 API，传入 `cursor`（首页传空字符串 `""`）和 `limit`
2. 将返回的 `info_list` 存入结果列表
3. 如果响应中 `is_end=false`，取出 `next_cursor` 作为下一次请求的 cursor
4. 重复 1-3 直到 `is_end=true`
5. 此时结果列表包含所有数据，再继续后续操作
